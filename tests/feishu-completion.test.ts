...
test('通知：整条消息均为注入时不覆盖上一条用户原文（辉哥定稿：请求只用用户原文）', async () => {
  const sent = []
  const notifier = makeNotifier(sent)
  ...
  // 先有真实输入，随后整条注入的消息不得覆盖它。
  notifier.observe(session, { type: 'user/message', data: { content: [{ type: 'text', text: '帮我把官网版本更新一下' }] } })
  notifier.observe(session, { type: 'user/message', data: { content: [{ type: 'text', text: injectedText }] } })
  notifier.observe(session, { type: 'turn/start', data: { turn: 1 } })
  ...
  // 请求内容加粗（辉哥定稿），且保持用户原文。
  assert.equal(request.text.content, '**请求**\n**帮我把官网版本更新一下**')
})

test('通知：后台任务完成通知（background job）不冒充用户请求', async () => {
  const sent = []
  const notifier = makeNotifier(sent)
  const session = { id: 's-job-notify' }
  const jobNotify = 'background job pwsh-4 (pwsh: pnpm typecheck *> \'D:\\项目\\tmp\\devforge-3410c-typecheck.log\'; "TSC=$LASTEXITCODE") finished [status: completed, exit code: 0]. Read its output with job_output.'
  notifier.observe(session, { type: 'user/message', data: { content: [{ type: 'text', text: '然后 有月的就改成 这样 5h-3:00 周-1d3h 月-28d20h' }] } })
  notifier.observe(session, { type: 'user/message', data: { content: [{ type: 'text', text: jobNotify }] } })
  notifier.observe(session, { type: 'turn/start', data: { turn: 1 } })
  notifier.observe(session, assistant('已发布 v0.34.10。'))
  notifier.observe(session, { type: 'turn/end', data: { turn: 1, reason: { kind: 'completed' } } })
  await sleep(80)
  assert.equal(sent.length, 1)
  const text = JSON.stringify(sent[0].card)
  assert.match(text, /然后 有月的就改成 这样/)
  assert.doesNotMatch(text, /background job/)
  assert.doesNotMatch(text, /job_output/)
})

test('通知：模型切换标注与上下文检查点同样不冒充用户请求', async () => {
  const sent = []
  const notifier = makeNotifier(sent)
  const session = { id: 's-model-changed' }
  notifier.observe(session, { type: 'user/message', data: { content: [{ type: 'text', text: '把侧栏卡片修一下' }] } })
  notifier.observe(session, { type: 'user/message', data: { content: [{ type: 'text', text: '[model changed: assistant turns above this point were generated by another model; the session continues.]' }] } })
  notifier.observe(session, { type: 'user/message', data: { content: [{ type: 'text', text: 'This is an automatically generated checkpoint condensing an earlier span of the conversation to free up context.' }] } })
  notifier.observe(session, { type: 'turn/start', data: { turn: 1 } })
  notifier.observe(session, assistant('已修复。'))
  notifier.observe(session, { type: 'turn/end', data: { turn: 1, reason: { kind: 'completed' } } })
  await sleep(80)
  assert.equal(sent.length, 1)
  const text = JSON.stringify(sent[0].card)
  assert.match(text, /把侧栏卡片修一下/)
  assert.doesNotMatch(text, /model changed/)
  assert.doesNotMatch(text, /automatically generated checkpoint/)
})
