...
import type { AccountSummary, GitAction, GitHubSettings, GitResult, RepoSummary } from '../../github/protocol.ts'
import css from './panel.module.css'

/** GitHub 工作台内部视图。 */
type GithubView = 'accounts' | 'repos' | 'git' | 'settings'
...
export function GithubTab({ api }: GithubTabProps): JSX.Element {
  ...
  // 账号表单状态。Token 从不从 Host 回填，避免凭据进入浏览器内存和 DOM。
  ...
  /** 验证 Token 并刷新账号对应的 GitHub 用户名。 */
  const testAccount = async (alias: string): Promise<void> => run(async () => {
    const result = await api.testGithubAccount(alias)
    if (!result.ok) throw new Error(result.error || 'GitHub 认证失败')
  ...
  const removeAccount = async (alias: string): Promise<void> => {
    if (!window.confirm('删除 GitHub 账号 ' + alias + '？')) return
    await run(async () => {
      await api.deleteGithubAccount(alias)
  ...
  const loadRepos = async (): Promise<void> => run(async () => {
    if (selected === '') throw new Error('请先配置 GitHub 账号')
    setRepos(await api.listGithubRepos(selected, repoSearch.trim() || undefined))
    setNotice('仓库列表已刷新')
  })
  ...
  const executeGit = async (): Promise<void> => run(async () => {
    if (action === 'push' && !settings.allowPush) throw new Error('Push 尚未授权，请先在安全设置中开启')
    if (action === 'push' && forcePush && !settings.allowForcePush) throw new Error('Force Push 尚未授权')
    const result = await api.runGithubGit({
      action,
  ...
  /** 保存设置时同步收紧 Force Push，禁止出现仅开强推但关闭普通 Push 的矛盾状态。 */
  const saveSettings = async (): Promise<void> => run(async () => {
    ...
    setSettings(await api.saveGithubSettings(normalized))
    setNotice('GitHub 设置已保存')
  })
  ...
  if (loading) return <div className={css['empty']} data-loading="">正在加载 GitHub 配置…</div>

  return (
    <section className={css['githubWorkspace']} aria-label="GitHub 工作台">
      <div className={css['subTabBar']} role="tablist" aria-label="GitHub 功能">
      ...
      {view === 'accounts' && (
        <div className={css['githubSplit']}>
          <section className={css['githubSection']}>
            ...
            <div className={css['form']}>
              <label className={css['field']}>
                ...
                <input className={css['input']} value={accountAlias} onChange={(event) => { setAccountAlias(event.target.value) }} placeholder="例如 work" autoComplete="off" />
              </label>
              <label className={css['field']}>
          ...
          <section className={css['githubSection']}>
            ...
            {accounts.length === 0 ? (
              <div className={css['empty']}>暂无 GitHub 账号</div>
            ) : (
              <div className={css['tableScroll']}>
                <table className={css['dataTable']}>
                  <thead><tr><th>别名</th><th>用户</th><th>Token</th><th>操作</th></tr></thead>
                  <tbody>
                    {accounts.map((account) => (
        ...
        <section className={css['githubSection']}>
          ...
          {action !== 'clone' && (
            <label className={css['field']}>
              ...
              <input className={css['input']} value={repoPath} onChange={(event) => { setRepoPath(event.target.value) }} placeholder="例如 D:/项目/repository" />
            </label>
          )}
          {action === 'clone' && (
            <div className={css['githubSplit']}>
              <label className={css['field']}>
                ...
                <input className={css['input']} value={remoteUrl} onChange={(event) => { setRemoteUrl(event.target.value) }} placeholder="https://github.com/owner/repo.git" />
              </label>
              <label className={css['field']}>
                <span className={css['fieldLabel']}>目标绝对目录</span>
                <input className={css['input']} value={destination} onChange={(event) => { setDestination(event.target.value) }} placeholder="例如 D:/项目/repository" />
              </label>
            </div>
          )}
          {action === 'commit' && (
            <label className={css['field']}>
              <span className={css['fieldLabel']}>Commit message</span>
          ...
          {action === 'push' && !settings.allowPush && <div className={css['banner']} data-kind="warning">Push 当前关闭，请先在“安全设置”中显式授权。</div>}
      ...
      {view === 'settings' && (
        <div className={css['githubSplit']}>
          <section className={css['githubSection']}>
            <h3 className={css['sectionTitle']}>GitHub 与 Git</h3>
            <div className={css['form']}>
              ...
              <label className={css['field']}><span className={css['fieldLabel']}>默认仓库目录</span><input className={css['input']} value={settings.defaultRepoDir || ''} onChange={(event) => { setSettings({ ...settings, defaultRepoDir: event.target.value }) }} placeholder="例如 D:/项目" /></label>
          ...
          <section className={css['githubSection']}>
            ...
            <div className={css['securityList']}>
              <label className={css['checkRow']}><input type="checkbox" checked={settings.autoFetchOnOpen} onChange={(event) => { setSettings({ ...settings, autoFetchOnOpen: event.target.checked }) }} />打开 GitHub 工作台时允许自动拉取</label>
              <label className={css['checkRow']}><input type="checkbox" checked={settings.allowPush} onChange={(event) => { setSettings({ ...settings, allowPush: event.target.checked, allowForcePush: event.target.checked ? settings.allowForcePush : false }) }} />允许 Agent 和面板 Push</label>
              <label className={css['checkRow']}><input type="checkbox" checked={settings.allowForcePush} disabled={!settings.allowPush} onChange={(event) => { setSettings({ ...settings, allowForcePush: event.target.checked }) }} />允许 Force Push</label>
            </div>
