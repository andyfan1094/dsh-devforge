...
const PROVIDERS: ProviderGuide[] = [
  {
    ...
    badge: '推荐入门',
    intro: '使用官方 Coding Plan 模型、额度看板和智谱 MCP（联网搜索、网页读取、Zread）。',
    steps: ['打开智谱开放平台并注册、登录，按页面提示完成实名认证与 Coding Plan 订阅。', '进入「个人编程套餐 → 套餐概览」（团队成员进入「团队编程套餐 → 我的套餐」），新建 API Key。', '回到「Coding Plan → 智谱 GLM」，粘贴 Key，点击「保存 Key」，再点击「完善模型接入」或「从官方拉取模型」。'],
    note: '受管凭据引用：ZAI_CODING_CN_API_KEY。Key 只写入 DSH 凭据存储，页面不会回显明文。',
