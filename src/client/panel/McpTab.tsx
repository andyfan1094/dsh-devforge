...
export function McpTab({ api }: { api: DevforgeApi }): JSX.Element {
  ...
  const card = { border: '1px solid rgba(128,128,128,0.3)', borderRadius: 8, padding: '10px 12px', minWidth: 0 } as const
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 13 }}>
      ...
      {servers.map((server) => (
        <div key={server.id} style={{ ...card, display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={row}>
            ...
            <span style={{ fontSize: 11, opacity: 0.7 }} title={status?.servers.find((item) => item.id === server.id)?.tools.map((tool) => tool.name).join('\n')}>{runtimeBadge(server, status)}</span>
            <div style={{ flex: 1 }} />
            <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }} title="停用后其全部工具从模型侧卸载">
        ...
        <div className={css.modalBackdrop} onClick={() => { setForm(null) }}>
          <div className={css.modal} role="dialog" aria-modal="true" aria-label="编辑 MCP 服务器" onClick={(event) => { event.stopPropagation() }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: '60vh', overflow: 'auto', paddingRight: 4 }}>
              <div style={row}>
                <label style={{ fontSize: 12, flex: 1, minWidth: 160 }}>显示名
                  <input className={css.input} style={{ width: '100%' }} value={form.name} placeholder="默认取命名空间" onChange={(e) => { setForm({ ...form, name: e.target.value }) }} />
                </label>
                <label style={{ fontSize: 12, flex: 1, minWidth: 160 }}>命名空间 serverName（工具名前缀）
                  <input className={css.input} style={{ width: '100%' }} value={form.serverName} placeholder="如 github（字母/数字/_/-，1-32 位）" onChange={(e) => { setForm({ ...form, serverName: e.target.value }) }} />
                </label>
              </div>
              <div style={row}>
                <label style={{ fontSize: 12 }}>传输类型
                  <select className={css.input} style={{ marginLeft: 6 }} value={form.transport} onChange={(e) => { setForm({ ...form, transport: e.target.value as McpFormState['transport'] }) }}>
                ...
                <label style={{ fontSize: 12, opacity: 0.85 }}>工具调用超时（毫秒）
                  <input className={css.input} style={{ width: 90, marginLeft: 6 }} type="number" min={5000} max={600000} step={1000} value={form.toolCallTimeoutMs} onChange={(e) => { setForm({ ...form, toolCallTimeoutMs: Number(e.target.value) || 60000 }) }} />
                </label>
              </div>

              {form.transport === 'stdio' ? (
                <>
                  <label style={{ fontSize: 12 }}>启动命令（如 npx / uvx / 本机可执行文件）
                    <input className={css.input} style={{ width: '100%', fontFamily: 'ui-monospace, monospace' }} value={form.command} placeholder="npx" onChange={(e) => { setForm({ ...form, command: e.target.value }) }} />
                  </label>
                  <label style={{ fontSize: 12 }}>参数（每行一个，不做 shell 插值）
                    <textarea className={css.input} style={{ width: '100%', minHeight: 54, fontFamily: 'ui-monospace, monospace' }} value={form.argsText} placeholder={'-y\n@modelcontextprotocol/server-github'} onChange={(e) => { setForm({ ...form, argsText: e.target.value }) }} />
                  </label>
                  <label style={{ fontSize: 12 }}>工作目录（留空 = 继承宿主）
                    <input className={css.input} style={{ width: '100%', fontFamily: 'ui-monospace, monospace' }} value={form.cwd} onChange={(e) => { setForm({ ...form, cwd: e.target.value }) }} />
                  </label>
                  <div style={{ fontSize: 12 }}>
                    环境变量
                    {form.envRows.map((entry, index) => (
                      <div key={index} style={{ ...row, marginTop: 4 }}>
                        <input className={css.input} style={{ flex: 1, minWidth: 120, fontFamily: 'ui-monospace, monospace' }} value={entry.key} placeholder="键，如 GITHUB_TOKEN" onChange={(e) => { setForm({ ...form, envRows: form.envRows.map((item, i) => (i === index ? { ...item, key: e.target.value } : item)) }) }} />
                        <input className={css.input} style={{ flex: 1, minWidth: 120, fontFamily: 'ui-monospace, monospace' }} type="password" value={entry.value} placeholder={entry.configured ? '已配置——留空保持原值' : '值'} onChange={(e) => { setForm({ ...form, envRows: form.envRows.map((item, i) => (i === index ? { ...item, value: e.target.value } : item)) }) }} />
                    ...
                    <button type="button" className={css.ghostButton} style={{ marginTop: 4 }} onClick={() => { setForm({ ...form, envRows: [...form.envRows, { key: '', value: '', configured: false }] }) }}>+ 添加变量</button>
                  </div>
                </>
              ) : (
                <>
                  <label style={{ fontSize: 12 }}>MCP 端点 URL
                    <input className={css.input} style={{ width: '100%', fontFamily: 'ui-monospace, monospace' }} value={form.url} placeholder="http://127.0.0.1:3000/mcp" onChange={(e) => { setForm({ ...form, url: e.target.value }) }} />
                  </label>
                  <div style={{ fontSize: 12 }}>
                    请求头（如 Authorization）
                    {form.headerRows.map((entry, index) => (
                      <div key={index} style={{ ...row, marginTop: 4 }}>
                        <input className={css.input} style={{ flex: 1, minWidth: 120, fontFamily: 'ui-monospace, monospace' }} value={entry.key} placeholder="键，如 Authorization" onChange={(e) => { setForm({ ...form, headerRows: form.headerRows.map((item, i) => (i === index ? { ...item, key: e.target.value } : item)) }) }} />
                        <input className={css.input} style={{ flex: 1, minWidth: 120, fontFamily: 'ui-monospace, monospace' }} type="password" value={entry.value} placeholder={entry.configured ? '已配置——留空保持原值' : '值'} onChange={(e) => { setForm({ ...form, headerRows: form.headerRows.map((item, i) => (i === index ? { ...item, value: e.target.value } : item)) }) }} />
                    ...
                    <button type="button" className={css.ghostButton} style={{ marginTop: 4 }} onClick={() => { setForm({ ...form, headerRows: [...form.headerRows, { key: '', value: '', configured: false }] }) }}>+ 添加请求头</button>
                  </div>
                </>
              )}
              <div style={{ fontSize: 11, opacity: 0.65 }}>工具注册后以 mcp__{form.serverName || '<命名空间>'}__&lt;工具名&gt; 呈现给模型；调用失败会如实报错，不会伪造成功。</div>
            </div>
      ...
      {testResult !== null && (
        <div className={css.modalBackdrop} onClick={() => { setTestResult(null) }}>
          <div className={css.modal} role="dialog" aria-modal="true" aria-label="MCP 连接测试结果" onClick={(event) => { event.stopPropagation() }}>
            ...
            <div className={css.modalContent} style={{ maxHeight: '50vh', overflow: 'auto' }}>
