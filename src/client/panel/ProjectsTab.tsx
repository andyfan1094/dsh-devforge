 ...
 * 「检测仓库」只读本机 .git 元数据自动回填远端与分支；所有请求统一经过
 * DevforgeApi；本页不保存任何凭据，服务器只记录主机别名。
 */
import { useCallback, useEffect, useState } from 'react'
import type { DevforgeApi } from '../api.ts'
import type { RemoteHostSummary } from '../../protocol.ts'
import type { ProjectDetectResult, ProjectEntry, RepoKind } from '../../projects/protocol.ts'
...
export function ProjectsTab({ api }: ProjectsTabProps): JSX.Element {
  ...
  return (
    <section className={css['githubWorkspace']} aria-label="项目面板">
      ...
      {editing === null && loading && <div className={css['empty']} data-loading="">正在加载项目清单…</div>}
      {editing === null && !loading && projects.length === 0 && (
        <div className={css['empty']}>还没有登记项目，点击「新建项目」开始。</div>
      )}

      {editing === null && projects.length > 0 && (
      ...
      {editing !== null && (
        <div className={css['form']}>
          ...
          <div className={css['field']}>
            <label className={css['fieldLabel']} htmlFor="project-path">项目路径（本机绝对路径）</label>
            <div className={css['inlineActions']}>
              <input
                id="project-path"
                className={css['input']}
                value={editing.path}
                placeholder="/Users/andyfan/Documents/ds/dsh-devforge"
                {...inputProps}
