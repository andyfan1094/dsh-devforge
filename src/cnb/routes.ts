/**
 * CNB 模块 HTTP 路由 —— /api/dsh-cnb 前缀，仅回环访问（loopback 围栏）。
 * 与 vendored GitHub 路由同构：accounts CRUD、account-test、repos、config、git。
 */
import type { IncomingMessage, ServerResponse } from 'node:http'
import type { WebRoute } from '@deepseek-ai/dsh-host-webserver'
import { CNB_API } from './protocol.ts'
...
export function makeRoutes(engine: CnbEngine): WebRoute[] {
  const accounts: WebRoute = { kind: 'exact', path: CNB_API.accounts, handler: async (req, res) => {
  ...
  const accountTest: WebRoute = { kind: 'exact', path: CNB_API.accountTest, handler: async (req, res) => { if (!guard(req, res, 'POST')) return; const body = await readJson(req); const alias = typeof body?.alias === 'string' ? body.alias : undefined; try { writeJson(res, 200, { result: await engine.testAccount(alias) }) } catch (error) { writeJson(res, 400, { error: errorMessage(error) }) } }}
  const repos: WebRoute = { kind: 'exact', path: CNB_API.repos, handler: async (req, res) => { if (!guard(req, res, 'GET')) return; try { writeJson(res, 200, { repos: await engine.listRepos(param(req, 'account'), param(req, 'query')) }) } catch (error) { writeJson(res, 400, { error: errorMessage(error) }) } }}
  const config: WebRoute = { kind: 'exact', path: CNB_API.config, handler: async (req, res) => {
    if (!isLoopbackRequest(req)) { writeJson(res, 403, { error: 'forbidden: loopback-only' }); return }
