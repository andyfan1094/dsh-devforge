...
export const BUILTIN_TEMPLATES: ForgeTemplate[] = [
  ...
  },
  {
    ...
    promptTemplate: [
      ...
      '需求描述：{requirements}',
      '硬性要求：',
      '1. 动工前先定设计基调（主色、灰阶、圆角、字体层级），并全部落成 CSS 变量或 Tailwind theme token（shadcn/ui 风格命名），任何颜色/间距/字号不得散写魔法值。',
      '2. 默认技术栈 Vite + Tailwind CSS；需求明确指定其他栈时可替换，但 token 原则不变。',
      '3. 页面必须响应式并支持暗色模式（token 双主题）；列表/详情页必须做加载、空、错误三种状态；可交互元素有 hover/active/focus-visible 三态。',
      '4. 信息密度按页面类型定档：看板/管理页默认紧凑密度（12 列栅格、表格行高 36~44px、分组间距 12~16px、渐进披露），一屏尽量多呈现信息且不拥挤；浏览型页面用舒适密度。',
      '5. 组件职责单一、样式与逻辑分离、关键流程写中文注释。',
      '6. 完成后按规范「UI 美观规范」一节逐条自查，交付摘要说明设计基调与自查结果；目标为可提交 Git 仓库时，验证通过后按规范自动创建中文提交。',
    ].join('\n'),
    builtin: true,
  },
]
