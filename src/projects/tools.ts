 ...
 *
 * 四个动作：list（清单）、resolve（按名称/路径关键词模糊解析单个项目）、
 * detail（按 id 取详情）、register（按本机路径登记/更新项目，自动检测 Git）。
...
export function devforgeProjectTool() {
  return defineTool({
    ...
    description: 'Resolve registered projects of this machine: list all, fuzzy-resolve one by name/path keyword, read details, or register a local project path (auto-detects Git remote/branch). ' +
      'Triggers: 我的项目, 项目列表, 项目解析, 项目登记, devforge_project, registered projects.',
    parameters: {
      action: { type: 'string', enum: ['list', 'resolve', 'detail', 'register'], description: 'list=项目清单；resolve=按关键词解析单个项目；detail=按 id 取详情；register=按本机路径登记/更新项目（自动检测 Git）。默认 list。' },
