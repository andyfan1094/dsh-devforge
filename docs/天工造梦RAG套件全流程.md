...
### 会话记忆层（M2.5，0.15.0 新增；0.28.0 升级为可信记忆治理）✅ 已完成

0.15.0 基础层：

- 自动沉淀：turn/end 事件驱动 + 去抖，取最近一轮用户/助手窗口（≤9k 字符）走默认模型路由提炼候选记忆（≤5 条）；失败只计数不扰会话。
- 主动注入：agent/pre-step 第一步检索记忆，以 plugin snapshot 用户消息追加进本轮（仿 dsh-time-context）。
- 红线：Mnemon/Hindsight 写入协议零改动；提炼失败不阻塞会话；凭据零落日志。

0.28.0 可信治理升级（候选优先，全部回归测试锁定）：

- **双域存储**：正文保持旧版 `memory.entry` 结构（旧版可读、回滚安全），信任/作用域/状态/证据/版本链写 `memory.meta` sidecar；关系链独立 `memory.relation`。
- **候选优先**：模型提炼与外部导入只产生 `memory.candidate`，pending 候选永不参与召回；激活仅两条路——治理面板人工批准，或宿主回读验证「用户明确要求记住」的直接原话。
- **作用域隔离**：宿主从会话 cwd + 项目登记表解析作用域（最长路径命中），召回硬过滤为全局 + 当前项目；项目红线不跨仓泄露。
- **混合召回**：词法（实体精确 + 中文二元组）与语义（embedding 向量）秩归一融合，embedding 失败只降级语义层；相对阈值拦截长尾弱相关。
- **冲突显式取代**：同事实键冲突的候选批准时必须选择取代对象，新条目写入、旧条目失效、关系记录同事务原子提交；钉选条目任何自动化不可触碰。
- **任务复盘**：`memory.episode` 记录目标/结果/工具/教训，「模型声明使用的记忆」必须与真实召回轨迹对账，防复盘造假。
- **反馈影子化**：`memory.feedback` 幂等记录（同一次注入同一条记忆只评一次），正反馈只做质量统计绝不自动改权；「有错误/已过期」反馈隔离非钉选条目等待人工处理。
- **做梦建议化**：做梦整理在生产只产生治理建议（proposalOnly），任何合并/归档必须人工在面板确认。
- **召回轨迹**：每次注入记录 `memory.recall`（查询哈希、逐命中分层分数、降级原因、延迟），面板可对真实注入的记忆直接反馈。
- **持久待处理窗口**：静默期内尚未提炼的轮次窗口落盘 `memory.pending-window`，Host 重启不丢工作。
- 红线不变：Mnemon/Hindsight 写入协议零改动；提炼失败不阻塞会话；凭据零落日志；模型置信度永远不等于激活权限。
