...
| Embedding | 智谱 embedding-3（首选，复用受管凭据）+ 方舟 doubao-embedding + OpenAI 中转备选；批量 + 文本 hash 缓存防重复计费 | 动工前真实调用验证 |
...
任务：智谱 rerank 接入 + LLM 重排兜底；多查询扩展 + HyDE；引用溯源（答案带编号标注，点开看文档名 + 标题路径 + 原文段）；rag_search 工具注册给全部会话；中文分词调优。
验收：会话内 rag_search 返回带出处卡片；重排前后召回对比在测试台可见。

### M3 项目知识库 + 记忆中枢连接器

任务：项目路径登记（联动现有项目面板）→ 目录扫描（尊重 .gitignore）→ 增量索引（文件 hash 变化才重嵌）；Mnemon Documents 与 Hindsight 知识页只读镜像拉取（接口验证顺序：HTTP 路由 → cordis inject 服务 → 导出文件兜底）；统一搜索页（多源并行查 + 来源徽章）。
验收：登记 dsh-devforge 自身项目，会话问「暂存实例怎么起」能召回约束文档原文；统一搜索一次返回多源结果。

### M4 工作流 + 多端 + 评估 + 生态

任务：工作流引擎（JSON DSL：改写/路由/多源检索/重排/生成/自评重试/联网兜底，节点可开关可调参）+ 运行历史；rag_run 工具；多端端到端（Mac 建库 → CNB 备份 → Windows 恢复 → 索引重建可用）；评估页（问答对评估集、Recall@K / MRR 跑分、配置对比）；服务工厂 RAG 服务模板。
验收：工作流一键运行出带引用答案；跨端恢复可用；评估页出对比表。
...
| 配置组 | 配置项 | 交互 |
|---|---|---|
| 向量模型 | 渠道（智谱/方舟/OpenAI 中转）+ 模型下拉 + 向量维度显示 + 测试连接按钮 | 切换时若已有库向量不兼容则触发守卫提示（见下） |
