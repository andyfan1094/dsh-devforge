...
12. **DSH 全 GUI 换肤**：操作台「皮肤」页签基于官方 `ctx.theme.register/overrideTokens` 一等 API 提供 13 套内置主题（8 浅 5 深，含 4 套紫色：薰衣草、雾紫、星紫夜、水晶紫夜；以及晨雾蓝、樱花粉、纸感暖、森林绿、蜜柑橙、午夜蓝、石墨黑、松林夜、墨玉青）+ 8 色强调色 + 可选壁纸（本地图片/http(s)/data URL，配遮罩浓度/模糊/显示方式）。选择持久化到 localStorage 跨重启自动跟随，深浅色与 GUI 自带模式自动联动；针对 ThemeRuntime 持久化偏好 schema 不接受第三方皮肤 id 的限制，引导期做 4 次 200ms 重试兜底，状态真相源切换为 `ctx.theme.getTheme().active.id` 保证 active 高亮与 GUI 实际着色始终一致；写入失败时 UI 顶部 banner 提示。
