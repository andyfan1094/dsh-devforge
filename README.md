...
12. **DSH 全 GUI 换肤**：操作台「皮肤」页签基于官方 `ctx.theme.register/overrideTokens` 一等 API 提供 10 套内置主题（5 浅 5 深：晨雾蓝、樱花粉、纸感暖、森林绿、蜜柑橙、午夜蓝、石墨黑、星紫夜、松林夜、墨玉青）+ 8 色强调色 + 可选壁纸（本地图片/http(s)/data URL，配遮罩浓度/模糊/显示方式）。选择持久化到 localStorage 跨重启自动跟随，深浅色与 GUI 自带模式自动联动；与社区 `dsh-dream-skin`、`dsh-skin` 等生态皮肤插件同源机制。
